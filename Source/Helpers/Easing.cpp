...
{
	return sinf((value - 1) * PI / 2.0f) + 1;
}

float Sinusoidal::Out(float value)
{
	return sinf(value * PI / 2.0f);
}

float Sinusoidal::InOut(float value)
{
	return 0.5f * (1.0f - cosf(value * PI));
}
...
{
	return sinf(13.0f * PI / 2.0f * value) * powf(2, 10 * (value - 1.0f));
}

float Elastic::Out(float value)
{
	return sinf(-13.0f * PI / 2.0f * (value + 1.0f)) * powf(2, -10 * value) + 1.0f;
}
...
float Elastic::InOut(float value)
{
	if (value < 0.5f)
	{
		return 0.5f * sinf(13.0f * PI / 2.0f * (2.0f * value)) * powf(2, 10 * ((2.0f * value) - 1.0f));
	}
	else
	{
		return 0.5f * (sinf(-13.0f * PI / 2.0f * ((2.0f * value - 1.0f) + 1.0f)) * powf(2, -10 * (2.0f * value - 1.0f)) + 2.0f);
	}
}

float Back::In(float value)
{
	return value * value * value - value * sinf(value * PI);
}
